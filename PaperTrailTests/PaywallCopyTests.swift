...
/// eligible×hasIntro matrix that decides the paywall's CTA label and fine
/// print for the selected plan. No StoreKit `Product` involved: `offersTrial`
/// already folds together "has an intro offer" and "is eligible for it",
/// so these tests only need to vary that single boolean plus plan/price.
struct PaywallCopyTests {

    // MARK: - ctaTitle

    @Test func ctaOffersTrialShowsStartFree() {
        #expect(PaywallCopy.ctaTitle(offersTrial: true, priceText: "S$39.98/yr") == "Start 2 weeks free")
    }

    @Test func ctaOffersTrialIgnoresMissingPrice() {
        // Trial copy never needs to quote a price up front.
        #expect(PaywallCopy.ctaTitle(offersTrial: true, priceText: nil) == "Start 2 weeks free")
    }

    @Test func ctaNoTrialShowsSubscribeWithPrice() {
        #expect(PaywallCopy.ctaTitle(offersTrial: false, priceText: "S$39.98/yr") == "Subscribe — S$39.98/yr")
        #expect(PaywallCopy.ctaTitle(offersTrial: false, priceText: "S$5.98/mo") == "Subscribe — S$5.98/mo")
    }

    @Test func ctaNoTrialNoPriceYetShowsBareSubscribe() {
        #expect(PaywallCopy.ctaTitle(offersTrial: false, priceText: nil) == "Subscribe")
    }

    // MARK: - finePrint

    @Test func finePrintTrialEligibleYearly() {
        let text = PaywallCopy.finePrint(offersTrial: true, isYearly: true, priceText: "S$39.98/yr")
    ...
    @Test func finePrintPaidYearlyKeepsRenewalPromise() {
        ...
        let text = PaywallCopy.finePrint(offersTrial: false, isYearly: true, priceText: "S$39.98/yr")
    ...
    @Test func finePrintMonthlyDropsRenewalPromise() {
        ...
        let text = PaywallCopy.finePrint(offersTrial: false, isYearly: false, priceText: "S$5.98/mo")
    ...
    @Test func finePrintMonthlyNeverShowsTrialCopyEvenIfOffersTrialWereTrue() {
        ...
        let text = PaywallCopy.finePrint(offersTrial: true, isYearly: false, priceText: "S$5.98/mo")
        #expect(text.hasPrefix("First 2 weeks free"))
    }

    @Test func finePrintMissingPriceFallsBackToEmDash() {
        let text = PaywallCopy.finePrint(offersTrial: false, isYearly: true, priceText: nil)
